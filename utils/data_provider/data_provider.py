...
flags.DEFINE_integer('max_image_large_side', 1280,
                     'max image size of training')
flags.DEFINE_integer('max_text_size', 800,
                     'if the text in the input image is bigger than this, then we resize'
                     'the image according to this')
flags.DEFINE_integer('min_text_area_size', 10,
                     'if the text area size is smaller than this, we ignore it during training')
flags.DEFINE_float('min_crop_side_ratio', 0.1,
...
def perimeter(poly):
    try:
        ...
        # logging.debug('perimeter:{}'.format(p))
        return p
    ...
    for i in range(len(scale_ratio)):
        ...
        for poly_idx, poly_tag in enumerate(zip(polys, tags)):
            ...
            if not len(shrinked_polys) and poly_idx not in ignore_poly_mark:
                logging.info("before shrink poly area:{} len(shrinked_poly) is 0,image {}".format(
...
def generator(input_size=512, batch_size=32,
    ...
    logging.info('{} training images in {}'.format(
    ...
    while True:
        ...
        for i in index:
            try:
                ...
                text_polys, text_tags = load_annotations(txt_fn)
                ...
                if np.random.rand() < background_ratio:
                    ...
                    if not len(seg_map_per_image):
                        logging.info("len(seg_map)==0 image: %d " % i)
                        continue
