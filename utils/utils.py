...
def im_resize_and_pad(img , h_resize, w_padding, black_padding = False, output_amounts = False):
    h, w, *_ = img.shape
    if h>=w :
        # Resize image#
        resize_orientation = "h"
        dim_resize = ((h_resize * w) // h, h_resize)
        img_resize = cv2.resize(img.astype(np.uint8), dim_resize)
        padding_size = w_padding - img_resize.shape[1]
        left = int(np.round(padding_size / 2))
        right = padding_size-left
        amount = (left, right)
        if black_padding :
            padded_img = cv2.copyMakeBorder(img_resize, 0, 0, left, right, cv2.BORDER_CONSTANT,
                                            value=[0, 0, 0])
        else :
            padded_img = cv2.copyMakeBorder(img_resize, 0, 0, left, right, cv2.BORDER_REPLICATE)
    else :
        resize_orientation = "w"
        dim_resize = (h_resize , (h_resize * h) // w)
        img_resize = cv2.resize(img.astype(np.uint8), dim_resize)
        padding_size = w_padding - img_resize.shape[0]
        up = int(np.round(padding_size / 2))
        down = padding_size - up
        amount = (up, down)
        if black_padding :
            padded_img = cv2.copyMakeBorder(img_resize, up, down, 0, 0, cv2.BORDER_CONSTANT,
                                            value=[0, 0, 0])
        else :
            padded_img = cv2.copyMakeBorder(img_resize, up, down, 0, 0, cv2.BORDER_REPLICATE)
    if output_amounts :
        retour = padded_img, resize_orientation, amount
    else :
        retour = padded_img
    return retour
